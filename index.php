...
.lastMessage {
    border-size: 5px;
    animation: 1s lastMessageAnim infinite;
}
  </style>
  <script>
  ...
  <?php 
    ...
    if($_GET['ecode'] == 'addsuccess') {
        if($left == 0) {
            echo "<div id='message' class='message lastMessage'><a href='javascript:void(0)' onclick='closeMsg'()'>&times;</a><img class='messageImg' src='./img/celebration.jpg' alt='celebration picture' /><p>That's it!  We are all done (at least until the next patch!)</p></div>";
        } else {
  ...
  ?>
    <div class="main" id="main">
        ...
        <?php 
            $sql = "SELECT * FROM spirits ORDER BY id LIMIT 60";
